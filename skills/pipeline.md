...
- **合集模式（用户偏好）**：一条视频装多个当天热点，整体约 2 分钟
  - 跑 `scripts/compile_daily.py --top N` 自动生成：选最热 N 个热点 → 每段 ~30s 短文案 → 拼成 2 分钟脚本
  - 每段结构：一句事件 + 一句赛道看点 + 一句互动提问 + 过渡"别急还有更劲爆的"
  - 结尾统一互动："这几个事你最关心哪个？评论区报个数"
  - 之后可人工润色每段（补细节/梗）再走配音
